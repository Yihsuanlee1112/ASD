...
class JsonProcess:
    ...
    def splitToVector3(self, vector3Series):
        ...
        df.columns = [f"{vector3Series.name}_X",
                      f"{vector3Series.name}_Y", f"{vector3Series.name}_Z"]
        df = df.astype("float")
        return df
    ...
    def splitToVector4(self, vector4Series):
        ...
        df.columns = [f"{vector4Series.name}_X", f"{vector4Series.name}_Y",
                      f"{vector4Series.name}_Z", f"{vector4Series.name}_W"]
        df = df.astype("float")
        return df
    ...
    def getStage(self, globalDataSeries):
        ...
        cleanStageData = globalDataSeries.str.extract(
            r"(?P<drop>_stage:)(?P<stage>[A-Za-z]+)")
